...
#data table
site_105_table <- data_table(sites, "105")

site_dt(site_105_table, "105")

#box plot for all analytes
bp_105 <- box_plot(site_105)
bp_105

#plot all VOCs at site
p_105 <- p_site(site_105, "105")
p_105

#plotly output but y axis not formatted correctly and don't know how to fix
ggplotly(p_105, tooltip = "text")

#TVOC for site by room
p_105_sum <- p_conc_room(site_105, "105 [Site Name]")
p_105_sum

#top 10 analyte concentrations for all locations
site_105_top <- top_n_analytes(site_105, 45)

site_105_top <- top_plot(site_105_top, "105", fill = "darkgreen")
site_105_top

#facet wrap by location
p_105_fctw <- fct_wrap(site_105, "105")
p_105_fctw 

#facet wrap by indoor location
p_105i_fctw <- fct_wrap(indoor_105, "105")
p_105i_fctw 
...
#plots for each room
#outdoor
p_105outdoor <- room_plot(outdoor_105, "105", "midnightblue", "Outdoor, rooftop")
p_105outdoor

#location1
p_location1 <- room_plot(location1, "105", "orchid", "location_1, [floor] [room type]")
p_location1

#location2
p_location2 <- room_plot(location2, "105", "chocolate4", "location_2, [floor] [room type]")
p_location2

#location3
p_location3 <- room_plot(location3, "105", "goldenrod2", "location_3, [floor] [room type]")
p_location3

#location4
p_location4 <- room_plot(location4, "105", "#50c878", "location_4, [floor] [room type]")
p_location4

#location5
p_location5 <- room_plot(location5, "105", "tomato2", "location_5, [floor] [room type]")
p_location5

#all rooms
grid.arrange(p_location1, p_location2, p_location4, p_location3,
             p_location5, p_105outdoor, 
             ncol = 3,
...
#outdoor
outdoor_top <- top_n_analytes(outdoor_105, 10)
...
p_105_bp <- r_box_plot(indoor_105)
p_105_bp

#indoor to outdoor ratio plot for all analytes
p_105_ratio <- r_p_site(indoor_105, "105")
p_105_ratio

#plotly ratio
ggplotly(p_105_ratio, tooltip = "text")

#ratio plot with facet wrap
p_105_r_fctw <- r_fct_wrap(indoor_105, "105")
p_105_r_fctw

#create object of the top 10 analytes by outdoor ratio then plot
#location1 outdoor ratio
location1_top_or <- top_n_or(indoor_105, "location_1", 10)
...
location2_top_or <- top_n_or(indoor_105, "location_1", 10)
...
location3_top_or <- top_n_or(indoor_105, "location_3", 10)
...
location4_top_or <- top_n_or(indoor_105, "location_4", 10)
...
location5_top_or <- top_n_or(indoor_105, "location_5", 10)
...
p_105_cat_fctw <- cat_fct_wrap(site_105, "105 (All Locations)")
p_105_cat_fctw

#indoor facet wrap by analyte class
p_105i_cat_fctw <- cat_fct_wrap(indoor_105, "105 (Indoor Locations)")
p_105i_cat_fctw
...
p_category(p_alcohol, "105 ", "Alcohols")
p_alcohol
...
p_category(p_aldehyde, "105 ", "Aldehydes")
p_aldehyde
...
p_category(p_straight_chain, "105 ", "Straight Chains")
p_straight_chain
...
p_category(p_aromatic, "105 ", "Aromatics")
p_aromatic
...
p_category(p_chlorinated, "105 ", "Chlorinated")
p_chlorinated
...
p_category(p_ketone, "105 ", "Ketones")
p_ketone
