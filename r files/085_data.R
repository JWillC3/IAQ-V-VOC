...
#data table
site_085_table <- data_table(sites, "085")

site_dt(site_085_table, "085")

#box plot for all analytes
bp_085 <- box_plot(site_085)
bp_085

#plot all VOCs at site
p_085 <- p_site(site_085, "085")
p_085

#plotly output but y axis not formatted correctly and don't know how to fix
ggplotly(p_085, tooltip = "text")

#TVOC for site by room
p_085_sum <- p_conc_room(site_085, "085 [Site Name]")
p_085_sum

#top 10 analyte concentrations for all locations
site_085_top <- top_n_analytes(site_085, 45)

site_085_top <- top_plot(site_085_top, "085", fill = "darkgreen")
site_085_top

#facet wrap by location
p_085_fctw <- fct_wrap(site_085, "085")
p_085_fctw 

#facet wrap by indoor location
p_085i_fctw <- fct_wrap(indoor_085, "085")
p_085i_fctw 
...
#plots for each room
#outdoor
p_085outdoor <- room_plot(outdoor_085, "085", "midnightblue", "Outdoor, rooftop")
p_085outdoor

#location1
p_location1 <- room_plot(location1, "085", "orchid", "location_1, [floor] [room type]")
p_location1

#location2
p_location2 <- room_plot(location2, "085", "chocolate4", "location_2, [floor] [room type]")
p_location2

#location3
p_location3 <- room_plot(location3, "085", "goldenrod2", "location_3, [floor] [room type]")
p_location3

#location4
p_location4 <- room_plot(location4, "085", "#50c878", "location_4, [floor] [room type]")
p_location4

#location5
p_location5 <- room_plot(location5, "085", "tomato2", "location_5, [floor] [room type]")
p_location5

#all rooms
grid.arrange(p_location1, p_location2, p_location4, p_location3,
             p_location5, p_085outdoor, 
             ncol = 3,
...
#outdoor
outdoor_top <- top_n_analytes(outdoor_085, 10)
...
p_085_bp <- r_box_plot(indoor_085)
p_085_bp

#indoor to outdoor ratio plot for all analytes
p_085_ratio <- r_p_site(indoor_085, "085")
p_085_ratio

#plotly ratio
ggplotly(p_085_ratio, tooltip = "text")

#ratio plot with facet wrap
p_085_r_fctw <- r_fct_wrap(indoor_085, "085")
p_085_r_fctw

#create object of the top 10 analytes by outdoor ratio then plot
#location1 outdoor ratio
location1_top_or <- top_n_or(indoor_085, "location_1", 10)
...
location2_top_or <- top_n_or(indoor_085, "location_1", 10)
...
location3_top_or <- top_n_or(indoor_085, "location_3", 10)
...
location4_top_or <- top_n_or(indoor_085, "location_4", 10)
...
location5_top_or <- top_n_or(indoor_085, "location_5", 10)
...
p_085_cat_fctw <- cat_fct_wrap(site_085, "085 (All Locations)")
p_085_cat_fctw

#indoor facet wrap by analyte class
p_085i_cat_fctw <- cat_fct_wrap(indoor_085, "085 (Indoor Locations)")
p_085i_cat_fctw
...
p_category(p_alcohol, "085 ", "Alcohols")
p_alcohol
...
p_category(p_aldehyde, "085 ", "Aldehydes")
p_aldehyde
...
p_category(p_straight_chain, "085 ", "Straight Chains")
p_straight_chain
...
p_category(p_aromatic, "085 ", "Aromatics")
p_aromatic
...
p_category(p_chlorinated, "085 ", "Chlorinated")
p_chlorinated
...
p_category(p_ketone, "085 ", "Ketones")
p_ketone
