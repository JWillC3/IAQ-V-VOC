...
#data table
site_086_table <- data_table(sites, "086")

site_dt(site_086_table, "086")

#box plot for all analytes
bp_086 <- box_plot(site_086)
bp_086

#plot all VOCs at site
p_086 <- p_site(site_086, "086")
p_086

#plotly output but y axis not formatted correctly and don't know how to fix
ggplotly(p_086, tooltip = "text")

#TVOC for site by room
p_086_sum <- p_conc_room(site_086, "086 [Site Name]")
p_086_sum

#top 10 analyte concentrations for all locations
site_086_top <- top_n_analytes(site_086, 45)

site_086_top <- top_plot(site_086_top, "086", fill = "darkgreen")
site_086_top

#facet wrap by location
p_086_fctw <- fct_wrap(site_086, "086")
p_086_fctw 

#facet wrap by indoor location
p_086i_fctw <- fct_wrap(indoor_086, "086")
p_086i_fctw 
...
#plots for each room
#outdoor
p_086outdoor <- room_plot(outdoor_086, "086", "midnightblue", "Outdoor, rooftop")
p_086outdoor

#location1
p_location1 <- room_plot(location1, "086", "orchid", "location_1, [floor] [room type]")
p_location1

#location2
p_location2 <- room_plot(location2, "086", "chocolate4", "location_2, [floor] [room type]")
p_location2

#location3
p_location3 <- room_plot(location3, "086", "goldenrod2", "location_3, [floor] [room type]")
p_location3

#location4
p_location4 <- room_plot(location4, "086", "#50c878", "location_4, [floor] [room type]")
p_location4

#location5
p_location5 <- room_plot(location5, "086", "tomato2", "location_5, [floor] [room type]")
p_location5

#all rooms
grid.arrange(p_location1, p_location2, p_location4, p_location3,
             p_location5, p_086outdoor, 
             ncol = 3,
...
#outdoor
outdoor_top <- top_n_analytes(outdoor_086, 10)
...
p_086_bp <- r_box_plot(indoor_086)
p_086_bp

#indoor to outdoor ratio plot for all analytes
p_086_ratio <- r_p_site(indoor_086, "086")
p_086_ratio

#plotly ratio
ggplotly(p_086_ratio, tooltip = "text")

#ratio plot with facet wrap
p_086_r_fctw <- r_fct_wrap(indoor_086, "086")
p_086_r_fctw

#create object of the top 10 analytes by outdoor ratio then plot
#location1 outdoor ratio
location1_top_or <- top_n_or(indoor_086, "location_1", 10)
...
location2_top_or <- top_n_or(indoor_086, "location_1", 10)
...
location3_top_or <- top_n_or(indoor_086, "location_3", 10)
...
location4_top_or <- top_n_or(indoor_086, "location_4", 10)
...
location5_top_or <- top_n_or(indoor_086, "location_5", 10)
...
p_086_cat_fctw <- cat_fct_wrap(site_086, "086 (All Locations)")
p_086_cat_fctw

#indoor facet wrap by analyte class
p_086i_cat_fctw <- cat_fct_wrap(indoor_086, "086 (Indoor Locations)")
p_086i_cat_fctw
...
p_category(p_alcohol, "086 ", "Alcohols")
p_alcohol
...
p_category(p_aldehyde, "086 ", "Aldehydes")
p_aldehyde
...
p_category(p_straight_chain, "086 ", "Straight Chains")
p_straight_chain
...
p_category(p_aromatic, "086 ", "Aromatics")
p_aromatic
...
p_category(p_chlorinated, "086 ", "Chlorinated")
p_chlorinated
...
p_category(p_ketone, "086 ", "Ketones")
p_ketone
