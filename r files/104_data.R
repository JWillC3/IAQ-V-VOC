...
#calculate ratios
indoor_104 <- indoor_104 %>%
  group_by(room_name, analyte) %>%
  ungroup() %>% 
  mutate(od_ratio = (indoor_104$conc./outdoor_104$conc.))
...
#data table
site_104_table <- data_table(sites, "104")

site_dt(site_104_table, "104")

#box plot for all analytes
bp_104 <- box_plot(site_104)
bp_104

#plot all VOCs at site
p_104 <- p_site(site_104, "104")
p_104

#plotly output but y axis not formatted correctly and don't know how to fix
ggplotly(p_104, tooltip = "text")

#TVOC for site by room
p_104_sum <- p_conc_room(site_104, "104 [Site Name]")
p_104_sum

#top 10 analyte concentrations for all locations
site_104_top <- top_n_analytes(site_104, 45)

site_104_top <- top_plot(site_104_top, "104", fill = "darkgreen")
site_104_top

#facet wrap by location
p_104_fctw <- fct_wrap(site_104, "104")
p_104_fctw 

#facet wrap by indoor location
p_104i_fctw <- fct_wrap(indoor_104, "104")
p_104i_fctw 
...
#plots for each room
#outdoor
p_104outdoor <- room_plot(outdoor_104, "104", "midnightblue", "Outdoor, rooftop")
p_104outdoor

#location1
p_location1 <- room_plot(location1, "104", "orchid", "location_1, [floor] [room type]")
p_location1

#location2
p_location2 <- room_plot(location2, "104", "chocolate4", "location_2, [floor] [room type]")
p_location2

#location3
p_location3 <- room_plot(location3, "104", "goldenrod2", "location_3, [floor] [room type]")
p_location3

#location4
p_location4 <- room_plot(location4, "104", "#50c878", "location_4, [floor] [room type]")
p_location4

#location5
p_location5 <- room_plot(location5, "104", "tomato2", "location_5, [floor] [room type]")
p_location5

#all rooms
grid.arrange(p_location1, p_location2, p_location4, p_location3,
             p_location5, p_104outdoor, 
             ncol = 3,
...
#outdoor
outdoor_top <- top_n_analytes(outdoor_104, 10)
...
p_104_bp <- r_box_plot(indoor_104)
p_104_bp

#indoor to outdoor ratio plot for all analytes
p_104_ratio <- r_p_site(indoor_104, "104")
p_104_ratio

#plotly ratio
ggplotly(p_104_ratio, tooltip = "text")

#ratio plot with facet wrap
p_104_r_fctw <- r_fct_wrap(indoor_104, "104")
p_104_r_fctw

#create object of the top 10 analytes by outdoor ratio then plot
#location1 outdoor ratio
location1_top_or <- top_n_or(indoor_104, "location_1", 10)
...
location2_top_or <- top_n_or(indoor_104, "location_1", 10)
...
location3_top_or <- top_n_or(indoor_104, "location_3", 10)
...
location4_top_or <- top_n_or(indoor_104, "location_4", 10)
...
location5_top_or <- top_n_or(indoor_104, "location_5", 10)
...
p_104_cat_fctw <- cat_fct_wrap(site_104, "104 (All Locations)")
p_104_cat_fctw

#indoor facet wrap by analyte class
p_104i_cat_fctw <- cat_fct_wrap(indoor_104, "104 (Indoor Locations)")
p_104i_cat_fctw
...
p_category(p_alcohol, "104 ", "Alcohols")
p_alcohol
...
p_category(p_aldehyde, "104 ", "Aldehydes")
p_aldehyde
...
p_category(p_straight_chain, "104 ", "Straight Chains")
p_straight_chain
...
p_category(p_aromatic, "104 ", "Aromatics")
p_aromatic
...
p_category(p_chlorinated, "104 ", "Chlorinated")
p_chlorinated
...
p_category(p_ketone, "104 ", "Ketones")
p_ketone
...
median_104 <- filter_and_summarize(indoor_104, analytes)
