...
#data table
site_107_table <- data_table(sites, "107")

site_dt(site_107_table, "107")

#box plot for all analytes
bp_107 <- box_plot(site_107)
bp_107

#plot all VOCs at site
p_107 <- p_site(site_107, "107")
p_107

#plotly output but y axis not formatted correctly and don't know how to fix
ggplotly(p_107, tooltip = "text")

#TVOC for site by room
p_107_sum <- p_conc_room(site_107, "107 [Site Name]")
p_107_sum

#top 10 analyte concentrations for all locations
site_107_top <- top_n_analytes(site_107, 45)

site_107_top <- top_plot(site_107_top, "107", fill = "darkgreen")
site_107_top

#facet wrap by location
p_107_fctw <- fct_wrap(site_107, "107")
p_107_fctw 

#facet wrap by indoor location
p_107i_fctw <- fct_wrap(indoor_107, "107")
p_107i_fctw 
...
#plots for each room
#outdoor
p_107outdoor <- room_plot(outdoor_107, "107", "midnightblue", "Outdoor, rooftop")
p_107outdoor

#location1
p_location1 <- room_plot(location1, "107", "orchid", "location_1, [floor] [room type]")
p_location1

#location2
p_location2 <- room_plot(location2, "107", "chocolate4", "location_2, [floor] [room type]")
p_location2

#location3
p_location3 <- room_plot(location3, "107", "goldenrod2", "location_3, [floor] [room type]")
p_location3

#location4
p_location4 <- room_plot(location4, "107", "#50c878", "location_4, [floor] [room type]")
p_location4

#location5
p_location5 <- room_plot(location5, "107", "tomato2", "location_5, [floor] [room type]")
p_location5

#all rooms
grid.arrange(p_location1, p_location2, p_location4, p_location3,
             p_location5, p_107outdoor, 
             ncol = 3,
...
#outdoor
outdoor_top <- top_n_analytes(outdoor_107, 10)
...
p_107_bp <- r_box_plot(indoor_107)
p_107_bp

#indoor to outdoor ratio plot for all analytes
p_107_ratio <- r_p_site(indoor_107, "107")
p_107_ratio

#plotly ratio
ggplotly(p_107_ratio, tooltip = "text")

#ratio plot with facet wrap
p_107_r_fctw <- r_fct_wrap(indoor_107, "107")
p_107_r_fctw

#create object of the top 10 analytes by outdoor ratio then plot
#location1 outdoor ratio
location1_top_or <- top_n_or(indoor_107, "location_1", 10)
...
location2_top_or <- top_n_or(indoor_107, "location_1", 10)
...
location3_top_or <- top_n_or(indoor_107, "location_3", 10)
...
location4_top_or <- top_n_or(indoor_107, "location_4", 10)
...
location5_top_or <- top_n_or(indoor_107, "location_5", 10)
...
p_107_cat_fctw <- cat_fct_wrap(site_107, "107 (All Locations)")
p_107_cat_fctw

#indoor facet wrap by analyte class
p_107i_cat_fctw <- cat_fct_wrap(indoor_107, "107 (Indoor Locations)")
p_107i_cat_fctw
...
p_category(p_alcohol, "107 ", "Alcohols")
p_alcohol
...
p_category(p_aldehyde, "107 ", "Aldehydes")
p_aldehyde
...
p_category(p_straight_chain, "107 ", "Straight Chains")
p_straight_chain
...
p_category(p_aromatic, "107 ", "Aromatics")
p_aromatic
...
p_category(p_chlorinated, "107 ", "Chlorinated")
p_chlorinated
...
p_category(p_ketone, "107 ", "Ketones")
p_ketone
