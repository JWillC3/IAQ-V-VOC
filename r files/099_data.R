...
#data table
site_099_table <- data_table(sites, "099")

site_dt(site_099_table, "099")

#box plot for all analytes
bp_099 <- box_plot(site_099)
bp_099

#plot all VOCs at site
p_099 <- p_site(site_099, "099")
p_099

#plotly output but y axis not formatted correctly and don't know how to fix
ggplotly(p_099, tooltip = "text")

#TVOC for site by room
p_099_sum <- p_conc_room(site_099, "099 [Site Name]")
p_099_sum

#top 10 analyte concentrations for all locations
site_099_top <- top_n_analytes(site_099, 45)

site_099_top <- top_plot(site_099_top, "099", fill = "darkgreen")
site_099_top

#facet wrap by location
p_099_fctw <- fct_wrap(site_099, "099")
p_099_fctw 

#facet wrap by indoor location
p_099i_fctw <- fct_wrap(indoor_099, "099")
p_099i_fctw 
...
#plots for each room
#outdoor
p_099outdoor <- room_plot(outdoor_099, "099", "midnightblue", "Outdoor, rooftop")
p_099outdoor

#location1
p_location1 <- room_plot(location1, "099", "orchid", "location_1, [floor] [room type]")
p_location1

#location2
p_location2 <- room_plot(location2, "099", "chocolate4", "location_2, [floor] [room type]")
p_location2

#location3
p_location3 <- room_plot(location3, "099", "goldenrod2", "location_3, [floor] [room type]")
p_location3

#location4
p_location4 <- room_plot(location4, "099", "#50c878", "location_4, [floor] [room type]")
p_location4

#location5
p_location5 <- room_plot(location5, "099", "tomato2", "location_5, [floor] [room type]")
p_location5

#all rooms
grid.arrange(p_location1, p_location2, p_location4, p_location3,
             p_location5, p_099outdoor, 
             ncol = 3,
...
#outdoor
outdoor_top <- top_n_analytes(outdoor_099, 10)
...
p_099_bp <- r_box_plot(indoor_099)
p_099_bp

#indoor to outdoor ratio plot for all analytes
p_099_ratio <- r_p_site(indoor_099, "099")
p_099_ratio

#plotly ratio
ggplotly(p_099_ratio, tooltip = "text")

#ratio plot with facet wrap
p_099_r_fctw <- r_fct_wrap(indoor_099, "099")
p_099_r_fctw

#create object of the top 10 analytes by outdoor ratio then plot
#location1 outdoor ratio
location1_top_or <- top_n_or(indoor_099, "location_1", 10)
...
location2_top_or <- top_n_or(indoor_099, "location_1", 10)
...
location3_top_or <- top_n_or(indoor_099, "location_3", 10)
...
location4_top_or <- top_n_or(indoor_099, "location_4", 10)
...
location5_top_or <- top_n_or(indoor_099, "location_5", 10)
...
p_099_cat_fctw <- cat_fct_wrap(site_099, "099 (All Locations)")
p_099_cat_fctw

#indoor facet wrap by analyte class
p_099i_cat_fctw <- cat_fct_wrap(indoor_099, "099 (Indoor Locations)")
p_099i_cat_fctw
...
p_category(p_alcohol, "099 ", "Alcohols")
p_alcohol
...
p_category(p_aldehyde, "099 ", "Aldehydes")
p_aldehyde
...
p_category(p_straight_chain, "099 ", "Straight Chains")
p_straight_chain
...
p_category(p_aromatic, "099 ", "Aromatics")
p_aromatic
...
p_category(p_chlorinated, "099 ", "Chlorinated")
p_chlorinated
...
p_category(p_ketone, "099 ", "Ketones")
p_ketone
