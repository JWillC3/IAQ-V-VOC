...
#data table
site_089_table <- data_table(sites, "089")

site_dt(site_089_table, "089")

#box plot for all analytes
bp_089 <- box_plot(site_089)
bp_089

#plot all VOCs at site
p_089 <- p_site(site_089, "089")
p_089

#plotly output but y axis not formatted correctly and don't know how to fix
ggplotly(p_089, tooltip = "text")

#TVOC for site by room
p_089_sum <- p_conc_room(site_089, "089 [Site Name]")
p_089_sum

#top 10 analyte concentrations for all locations
site_089_top <- top_n_analytes(site_089, 45)

site_089_top <- top_plot(site_089_top, "089", fill = "darkgreen")
site_089_top

#facet wrap by location
p_089_fctw <- fct_wrap(site_089, "089")
p_089_fctw 

#facet wrap by indoor location
p_089i_fctw <- fct_wrap(indoor_089, "089")
p_089i_fctw 
...
#plots for each room
#outdoor
p_089outdoor <- room_plot(outdoor_089, "089", "midnightblue", "Outdoor, rooftop")
p_089outdoor

#location1
p_location1 <- room_plot(location1, "089", "orchid", "location_1, [floor] [room type]")
p_location1

#location2
p_location2 <- room_plot(location2, "089", "chocolate4", "location_2, [floor] [room type]")
p_location2

#location3
p_location3 <- room_plot(location3, "089", "goldenrod2", "location_3, [floor] [room type]")
p_location3

#location4
p_location4 <- room_plot(location4, "089", "#50c878", "location_4, [floor] [room type]")
p_location4

#location5
p_location5 <- room_plot(location5, "089", "tomato2", "location_5, [floor] [room type]")
p_location5

#all rooms
grid.arrange(p_location1, p_location2, p_location4, p_location3,
             p_location5, p_089outdoor, 
             ncol = 3,
...
#outdoor
outdoor_top <- top_n_analytes(outdoor_089, 10)
...
p_089_bp <- r_box_plot(indoor_089)
p_089_bp

#indoor to outdoor ratio plot for all analytes
p_089_ratio <- r_p_site(indoor_089, "089")
p_089_ratio

#plotly ratio
ggplotly(p_089_ratio, tooltip = "text")

#ratio plot with facet wrap
p_089_r_fctw <- r_fct_wrap(indoor_089, "089")
p_089_r_fctw

#create object of the top 10 analytes by outdoor ratio then plot
#location1 outdoor ratio
location1_top_or <- top_n_or(indoor_089, "location_1", 10)
...
location2_top_or <- top_n_or(indoor_089, "location_1", 10)
...
location3_top_or <- top_n_or(indoor_089, "location_3", 10)
...
location4_top_or <- top_n_or(indoor_089, "location_4", 10)
...
location5_top_or <- top_n_or(indoor_089, "location_5", 10)
...
p_089_cat_fctw <- cat_fct_wrap(site_089, "089 (All Locations)")
p_089_cat_fctw

#indoor facet wrap by analyte class
p_089i_cat_fctw <- cat_fct_wrap(indoor_089, "089 (Indoor Locations)")
p_089i_cat_fctw
...
p_category(p_alcohol, "089 ", "Alcohols")
p_alcohol
...
p_category(p_aldehyde, "089 ", "Aldehydes")
p_aldehyde
...
p_category(p_straight_chain, "089 ", "Straight Chains")
p_straight_chain
...
p_category(p_aromatic, "089 ", "Aromatics")
p_aromatic
...
p_category(p_chlorinated, "089 ", "Chlorinated")
p_chlorinated
...
p_category(p_ketone, "089 ", "Ketones")
p_ketone
