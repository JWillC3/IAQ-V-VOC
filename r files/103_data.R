...
#data table
site_103_table <- data_table(sites, "103")

site_dt(site_103_table, "103")

#box plot for all analytes
bp_103 <- box_plot(site_103)
bp_103

#plot all VOCs at site
p_103 <- p_site(site_103, "103")
p_103

#plotly output but y axis not formatted correctly and don't know how to fix
ggplotly(p_103, tooltip = "text")

#TVOC for site by room
p_103_sum <- p_conc_room(site_103, "103 [Site Name]")
p_103_sum

#top 10 analyte concentrations for all locations
site_103_top <- top_n_analytes(site_103, 45)

site_103_top <- top_plot(site_103_top, "103", fill = "darkgreen")
site_103_top

#facet wrap by location
p_103_fctw <- fct_wrap(site_103, "103")
p_103_fctw 

#facet wrap by indoor location
p_103i_fctw <- fct_wrap(indoor_103, "103")
p_103i_fctw 
...
#plots for each room
#outdoor
p_103outdoor <- room_plot(outdoor_103, "103", "midnightblue", "Outdoor, rooftop")
p_103outdoor

#location1
p_location1 <- room_plot(location1, "103", "orchid", "location_1, [floor] [room type]")
p_location1

#location2
p_location2 <- room_plot(location2, "103", "chocolate4", "location_2, [floor] [room type]")
p_location2

#location3
p_location3 <- room_plot(location3, "103", "goldenrod2", "location_3, [floor] [room type]")
p_location3

#location4
p_location4 <- room_plot(location4, "103", "#50c878", "location_4, [floor] [room type]")
p_location4

#location5
p_location5 <- room_plot(location5, "103", "tomato2", "location_5, [floor] [room type]")
p_location5

#all rooms
grid.arrange(p_location1, p_location2, p_location4, p_location3,
             p_location5, p_103outdoor, 
             ncol = 3,
...
#outdoor
outdoor_top <- top_n_analytes(outdoor_103, 10)
...
p_103_bp <- r_box_plot(indoor_103)
p_103_bp

#indoor to outdoor ratio plot for all analytes
p_103_ratio <- r_p_site(indoor_103, "103")
p_103_ratio

#plotly ratio
ggplotly(p_103_ratio, tooltip = "text")

#ratio plot with facet wrap
p_103_r_fctw <- r_fct_wrap(indoor_103, "103")
p_103_r_fctw

#create object of the top 10 analytes by outdoor ratio then plot
#location1 outdoor ratio
location1_top_or <- top_n_or(indoor_103, "location_1", 10)
...
location2_top_or <- top_n_or(indoor_103, "location_1", 10)
...
location3_top_or <- top_n_or(indoor_103, "location_3", 10)
...
location4_top_or <- top_n_or(indoor_103, "location_4", 10)
...
location5_top_or <- top_n_or(indoor_103, "location_5", 10)
...
p_103_cat_fctw <- cat_fct_wrap(site_103, "103 (All Locations)")
p_103_cat_fctw

#indoor facet wrap by analyte class
p_103i_cat_fctw <- cat_fct_wrap(indoor_103, "103 (Indoor Locations)")
p_103i_cat_fctw
...
p_category(p_alcohol, "103 ", "Alcohols")
p_alcohol
...
p_category(p_aldehyde, "103 ", "Aldehydes")
p_aldehyde
...
p_category(p_straight_chain, "103 ", "Straight Chains")
p_straight_chain
...
p_category(p_aromatic, "103 ", "Aromatics")
p_aromatic
...
p_category(p_chlorinated, "103 ", "Chlorinated")
p_chlorinated
...
p_category(p_ketone, "103 ", "Ketones")
p_ketone
