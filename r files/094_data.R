...
#data table
site_094_table <- data_table(sites, "094")

site_dt(site_094_table, "094")

#box plot for all analytes
bp_094 <- box_plot(site_094)
bp_094

#plot all VOCs at site
p_094 <- p_site(site_094, "094")
p_094

#plotly output but y axis not formatted correctly and don't know how to fix
ggplotly(p_094, tooltip = "text")

#TVOC for site by room
p_094_sum <- p_conc_room(site_094, "094 [Site Name]")
p_094_sum

#top 10 analyte concentrations for all locations
site_094_top <- top_n_analytes(site_094, 45)

site_094_top <- top_plot(site_094_top, "094", fill = "darkgreen")
site_094_top

#facet wrap by location
p_094_fctw <- fct_wrap(site_094, "094")
p_094_fctw 

#facet wrap by indoor location
p_094i_fctw <- fct_wrap(indoor_094, "094")
p_094i_fctw 
...
#plots for each room
#outdoor
p_094outdoor <- room_plot(outdoor_094, "094", "midnightblue", "Outdoor, rooftop")
p_094outdoor

#location1
p_location1 <- room_plot(location1, "094", "orchid", "location_1, [floor] [room type]")
p_location1

#location2
p_location2 <- room_plot(location2, "094", "chocolate4", "location_2, [floor] [room type]")
p_location2

#location3
p_location3 <- room_plot(location3, "094", "goldenrod2", "location_3, [floor] [room type]")
p_location3

#location4
p_location4 <- room_plot(location4, "094", "#50c878", "location_4, [floor] [room type]")
p_location4

#location5
p_location5 <- room_plot(location5, "094", "tomato2", "location_5, [floor] [room type]")
p_location5

#all rooms
grid.arrange(p_location1, p_location2, p_location4, p_location3,
             p_location5, p_094outdoor, 
             ncol = 3,
...
#outdoor
outdoor_top <- top_n_analytes(outdoor_094, 10)
...
p_094_bp <- r_box_plot(indoor_094)
p_094_bp

#indoor to outdoor ratio plot for all analytes
p_094_ratio <- r_p_site(indoor_094, "094")
p_094_ratio

#plotly ratio
ggplotly(p_094_ratio, tooltip = "text")

#ratio plot with facet wrap
p_094_r_fctw <- r_fct_wrap(indoor_094, "094")
p_094_r_fctw

#create object of the top 10 analytes by outdoor ratio then plot
#location1 outdoor ratio
location1_top_or <- top_n_or(indoor_094, "location_1", 10)
...
location2_top_or <- top_n_or(indoor_094, "location_1", 10)
...
location3_top_or <- top_n_or(indoor_094, "location_3", 10)
...
location4_top_or <- top_n_or(indoor_094, "location_4", 10)
...
location5_top_or <- top_n_or(indoor_094, "location_5", 10)
...
p_094_cat_fctw <- cat_fct_wrap(site_094, "094 (All Locations)")
p_094_cat_fctw

#indoor facet wrap by analyte class
p_094i_cat_fctw <- cat_fct_wrap(indoor_094, "094 (Indoor Locations)")
p_094i_cat_fctw
...
p_category(p_alcohol, "094 ", "Alcohols")
p_alcohol
...
p_category(p_aldehyde, "094 ", "Aldehydes")
p_aldehyde
...
p_category(p_straight_chain, "094 ", "Straight Chains")
p_straight_chain
...
p_category(p_aromatic, "094 ", "Aromatics")
p_aromatic
...
p_category(p_chlorinated, "094 ", "Chlorinated")
p_chlorinated
...
p_category(p_ketone, "094 ", "Ketones")
p_ketone
