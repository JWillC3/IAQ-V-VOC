---
title: "Site 066 - Homeward Bound VOC Data"
author: "Will Clagett"
date: "2024-01-02"
...
# **VOC Data**
## **Site 066 - Homeward Bound**
## **April 25 - May 5, 2023**
## *Grand Junction, CO*
```{r, site_photo, out.width="50%"}

include_graphics("../graphics/066.png")
...
voc_plot <- ggplot(voc, aes(x = reorder(analyte, conc.),
  ...
  labs(x = "Analytes", y = "Concentration\n(VOC ppbv or methane ppmv)") +
   ggtitle("Site 066 - Homeward Bound",
          "Apr. 25 - May 05, 2023")
...
voc_plot2 <- ggplot(voc2, aes(x = reorder(analyte, conc.),
  ...
  xlab("Analytes") +
  ylab("Concentration\n(VOC ppbv or methane ppmv)") +
  ggtitle("Site 066 - Homeward Bound, Summa Cannister Deployment. Mmm. XX - Mmm. XX, YYYY")
...
l1_plot <- barracks_n %>%
  ...
  ggtitle("066 - Homeward Bound: Barracks North",
          "first floor sleeping room shared with dining")
l1_plot +
...
l2_plot <- barracks_s %>%
  ...
  labs(x = "Analytes",
       y = expression(atop("Concentration",
                           paste("(VOC ppbv or methane ppmv)")))) +
  ggtitle("066 - Homeward Bound: Barracks South",
          "first floor sleeping room")
l2_plot +
  theme(panel.grid.minor = element_line(linetype = "dashed"))
...
Dining
```{r dining_plot, fig.width = 10, fig.height = 5, warning=FALSE, message=FALSE}
...
l3_plot <- dining %>%
  ...
  labs(x = "Analytes",
       y = expression(atop("Concentration",
                           paste("(VOC ppbv or methane ppmv)")))) +
  ggtitle("066 - Homeward Bound: Dining",
...
l4_plot <- office %>%
  ...
  labs(x = "Analytes",
       y = expression(atop("Concentration",
                           paste("(VOC ppbv or methane ppmv)")))) +
  ggtitle("066 Homeward Bound: Office",
          "first floor office")
l4_plot +
  theme(panel.grid.minor = element_line(linetype = "dashed"))
...
l5_plot <- outdoor %>%
  ...
  labs(x = "Analytes",
       y = expression(atop("Concentration",
                           paste("(VOC ppbv or methane ppmv)")))) +
  ggtitle("066 - Homeward Bound: Outdoor",
          "rooftop")
l5_plot +
  theme(panel.grid.minor = element_line(linetype = "dashed"))
...
Welcome Room
```{r welcome_plot, fig.width = 10, fig.height = 5, warning=FALSE, message=FALSE}
...
l6_plot <- welcome %>%
  ...
  labs(x = "Analytes",
       y = expression(atop("Concentration",
                           paste("(VOC ppbv or methane ppmv)")))) +
  ggtitle("066 - Homeward Bound: Welcome Room,",
          "first floor lobby")
l6_plot +
  theme(panel.grid.minor = element_line(linetype = "dashed"))
...
welcome_top <- welcome %>% 
  group_by(analyte) %>% 
  arrange(desc(conc.))
welcome_top <- top_n(ungroup(welcome_top), 5, conc.)


```


```{r barracks_n_t, warning=FALSE, message=FALSE}
p_barracks_n_top <- barracks_n_top %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, conc.), y = conc.),
           stat = "identity", fill = "orchid") +
  labs(x = "Barracks North", y = "") +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
...
```{r barracks_s_t, warning=FALSE, message=FALSE}

p_barracks_s_top <- barracks_s_top %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, conc.), y = conc.),
           stat = "identity", fill = "chocolate4") +
  labs(x = "Barracks South", y = "") +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
...
```{r, welcome_t, warning=FALSE, message=FALSE}
...
grid.arrange(p_barracks_n_top, p_barracks_s_top, p_dining_top, p_office_top,
             p_outdoor_top,p_welcome_top,
             ncol = 3, nrow = 2,
...
#barracks_n od ratio
barracks_n_top_or <- indoor %>% 
  filter(ID == "Barracks North") %>%
  group_by(analyte) %>% 
  arrange(desc(od_ratio))
barracks_n_top_or <- top_n(ungroup(barracks_n_top_or), 5, od_ratio) 

#barracks_s od ratio
barracks_s_top_or <- indoor %>% 
  filter(ID == "Barracks South") %>% 
  group_by(analyte) %>% 
  arrange(desc(od_ratio))
barracks_s_top_or <- top_n(ungroup(barracks_s_top_or), 5, od_ratio)
...
#welcome od ratio
welcome_top_or <- indoor %>% 
  filter(ID == "Welcome Room") %>% 
  group_by(analyte) %>% 
  arrange(desc(od_ratio))
welcome_top_or <- top_n(ungroup(welcome_top_or), 5, od_ratio)


```


```{r barracks_n_or_t, warning=FALSE, message=FALSE}

p_barracks_n_top_or <- barracks_n_top_or %>% 
  ggplot() +
...
```{r barracks_s_or_t, warning=FALSE, message=FALSE}

p_barracks_s_top_or <- barracks_s_top_or %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, od_ratio), y = od_ratio),
           stat = "identity", fill = "chocolate4") +
  labs(x = "Barracks South", y = "") +
...
```{r welcome_or_t, warning=FALSE, message=FALSE}

p_welcome_top_or <- welcome_top_or %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, od_ratio), y = od_ratio),
           stat = "identity", fill = "tomato2") +
  labs(x = "Welcome Room", y = "") +
...
grid.arrange(p_barracks_n_top_or, p_barracks_s_top_or, p_dining_top_or,
             p_office_top_or, p_welcome_top_or,
             ncol = 3, nrow = 2,
