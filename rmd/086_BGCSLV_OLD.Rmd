...
Summa Canisters were deployed in three locations at the site:
...
site_086 <- (read_csv(file = "./data/site_086_summa_data.csv")) %>% 
  slice(2:5, 7:8) %>%
  select(1,7:67)
...
voc_plot <- ggplot(voc, aes(x = reorder(analyte, conc.),
  ...
  ggtitle("Site 086 Summa Cannister Deployment",
...
voc_combo_point <- ggplot(indoor, aes(color = ID, x = analyte, y = conc.)) +
  ...
  ggtitle("Site 086: BGSLV Summa Cannister Deployment",
          "Jul. 27 - Aug. 03, 2023. Alamosa, CO")
voc_combo_point+
  scale_color_manual(values = c("#fde725", "#7ad151",
                                  "#22a884", "#2a788e", "#414487",
                                  "#440154"),
                       labels = c("music room", "outdoor",
                                       "social rec")) +
...
gr_lava <- ggplot(voc, aes(x = reorder(analyte, conc.),
  ...
  ggtitle("Site 086: BGSLV Summa Cannister Deployment",
          "Jul. 27 - Aug. 03, 2023. Alamosa, CO")
gr_lava +
  scale_color_manual(values = c("#261e1b","#af6f70","#fbc117"),
                     labels = c("music room", "outdoor", "social rec")) +
...
indoor_fctg +
  scale_color_manual(values = c("#3090C7", "#C04000"),
                     labels = c("music room", "social rec")) +
  theme(legend.title = element_text(color = "black", size = 10),
        legend.text = element_text(color = "black"))
...
social_rec_top <- social_rec %>% 
  group_by(analyte) %>% 
  arrange(desc(conc.))
social_rec_top <- top_n(ungroup(social_rec_top), 8, conc.) 
#music room
...
p_social_rec_top <- social_rec_top %>% 
  ...
  labs(x = "social rec", y = "Concentration") +
  scale_y_log10(labels = trans_format(`log10`, math_format(10^.x)))+
  theme_bw() +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1))
...
p_music_room_top <- music_room_top %>% 
  ...
  labs(x = "music room", y = "Concentration") +
...
grid.arrange(p_social_rec_top, p_music_room_top, p_od_top,
             ncol = 2, nrow = 2,
             top = "Top 5 Analytes in Each Location")

```

## Top 5 Outdoor Ratio Plots by Location
```{r top_five_data, include=FALSE}
#social rec
or_social_rec_top <- social_rec %>% 
  group_by(analyte) %>% 
  arrange(desc(od_ratio))
or_social_rec_top <- top_n(ungroup(social_rec_top), 5, od_ratio) 
#music room
or_music_room_top <- music_room %>% 
  group_by(analyte) %>% 
  arrange(desc(od_ratio))
or_music_room_top <- top_n(ungroup(music_room_top), 5, od_ratio)

```


```{r social rec_t, warning=FALSE, message=FALSE}
p_or_social_rec_top <- or_social_rec_top %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, od_ratio), y = od_ratio),
           stat = "identity", fill = "#065143") +
  labs(x = "social rec", y = "Outdoor Ratio") +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
               labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1))

p_or_social_rec_top + theme(
  panel.background = element_rect(fill = "lightblue",
                                colour = "lightblue",
                                size = 0.5, linetype = "solid"))
...
```{r music room_t, warning=FALSE, message=FALSE}

p_or_music_room_top <- music_room_top %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, od_ratio), y = od_ratio),
           stat = "identity", fill = "#A74AC7") +
  labs(x = "music room", y = "outdoor ratio") +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
               labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1))

p_or_music_room_top + theme(
  panel.background = element_rect(fill = "lightblue",
                                colour = "lightblue",
                                size = 0.5, linetype = "solid"))
...
grid.arrange(p_or_social_rec_top, p_or_music_room_top,
             ncol = 2,
             top = "Top 5 Analytes in Each Location")
...
ratio_plot <- ggplot(indoor, aes(x = reorder(analyte, od_ratio),
  ...
  xlab("Analytes") +
  ylab("Indoor to Outdoor Ratios") +
  ggtitle("Site 086 Summa Cannister Deployment: Jul. 27 - Aug. 03, 2023. Alamosa, CO")
ratio_plot + theme(
...
yellowrainbow + scale_color_manual(values = c("#EDDA74","#024b30"),
                                   labels = c("music room", "social rec")) +
  theme(legend.title = element_text(color = "black", size = 10),
        legend.text = element_text(color = "#261e1b"))
...
yellowrainbow + scale_color_manual(values = c("orchid", "chocolate4",
                                   ...
                                   labels = c("music room", "outoodr", "social rec")) +
  theme(legend.title = element_text(color = "black", size = 10),
        legend.text = element_text(color = "#261e1b"))
...
yellowrainbow + scale_colour_viridis(discrete = TRUE,
                                   labels = c("social rec", "music room",
                                              "location3", 
                                              "location4", "location5")) +
  theme(legend.title = element_text(color = "black", size = 10),
        legend.text = element_text(color = "#261e1b"))
...
#correlations
cor(outdoor$conc., social_rec$conc., method = "spearman")
