...
#include_graphics("../graphics/103.jpg")
...
site_103 <- (read_csv(file = "../data/site_103.csv")) %>% 
  slice(2:7) %>%
  select(1,7:67)
...
outdoor <- voc %>% 
  filter(ID == "Outdoor")
#second_floor_apt
second_floor_apt <- voc %>% 
  filter(ID == "Second Floor Apt")
#location 6
third_floor_apt <- voc %>% 
  filter(ID == "Third Floor Apt")
...
voc_plot2 <- ggplot(voc2, aes(x = reorder(analyte, conc.),
  ...
  xlab("Analytes") +
  ylab("Concentration\n(VOC ppbv or methane ppmv)") +
  ggtitle("Site 103 - Karis, Summa Cannister Deployment. Mmm. XX - Mmm. XX, YYYY")
...
l1_plot <- basement %>%
  ...
  ggtitle("103 - Karis: Basement",
          "basement level TV room")
l1_plot +
...
Kitchen
```{r kitcehn_plot, fig.width = 10, fig.height = 5, warning=FALSE, message=FALSE}
...
Second Floor Apt.
```{r second_floor_apt_plot, fig.width = 10, fig.height = 5, warning=FALSE, message=FALSE}

#second_floor_apt
l5_plot <- second_floor_apt %>%
  ggplot(aes(x = reorder(analyte, conc.), y = conc., color = ID)) +
  ...
  labs(x = "Analytes",
       y = expression(atop("Concentration",
                           paste("(VOC ppbv or methane ppmv)")))) +
  ggtitle("103 - Karis: Second Floor Apt.",
...
Third Floor Apt.
```{r third_floor_apt_plot, fig.width = 10, fig.height = 5, warning=FALSE, message=FALSE}

#third_floor_apt
l6_plot <- third_floor_apt %>%
  ggplot(aes(x = reorder(analyte, conc.), y = conc., color = ID)) +
  ...
  labs(x = "Analytes",
       y = expression(atop("Concentration",
                           paste("(VOC ppbv or methane ppmv)")))) +
  ggtitle("103 - Karis: Third Floor Apt.,",
...
voc_fctg <- ggplot(voc, aes(x = reorder(analyte, conc.),
  ...
  labs(x = "Analytes Grouped by VOC Categories",
       y = expression(atop("Concentration",
                           paste("(VOC ppbv or methane ppmv)")))) +
  ggtitle("Site 103 - Karis, Summa Cannister Deployment",
...
indoor_fctg <- ggplot(indoor, aes(x = reorder(analyte, conc.),
  ...
  labs(x = "Analytes Grouped by VOC Categories",
       y = expression(atop("Concentration",
                           paste("(VOC ppbv or methane ppmv)")))) +
  ggtitle("Site 103 - Karis, Indoor Summa Cannister Deployment",
          "Grouped by Analyte Class")
indoor_fctg +
  scale_color_manual(name = "Room ID",
                     values = c("orchid", "chocolate4", "goldenrod2","#50C878",
...
gallery_top <- gallery %>% 
  group_by(analyte) %>% 
  arrange(desc(conc.))
gallery_top <- top_n(ungroup(gallery_top), 5, conc.)

#kitcehn
kitcehn_top <- kitcehn %>% 
  group_by(analyte) %>% 
  arrange(desc(conc.))
kitcehn_top <- top_n(ungroup(kitcehn_top), 5, conc.)
...
#second_floor_apt
second_floor_apt_top <- second_floor_apt %>% 
  group_by(analyte) %>% 
  arrange(desc(conc.))
second_floor_apt_top <- top_n(ungroup(second_floor_apt_top), 5, conc.)

#third_floor_apt
third_floor_apt_top <- third_floor_apt %>% 
  group_by(analyte) %>% 
  arrange(desc(conc.))
third_floor_apt_top <- top_n(ungroup(third_floor_apt_top), 5, conc.)
...
```{r gallery_t, warning=FALSE, message=FALSE}

p_gallery_top <- gallery_top %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, conc.), y = conc.),
...
```{r kitcehn_t, warning=FALSE, message=FALSE}

p_kitcehn_top <- kitcehn_top %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, conc.), y = conc.),
...
```{r second_floor_apt_t, warning=FALSE, message=FALSE}

p_second_floor_apt_top <- second_floor_apt_top %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, conc.), y = conc.),
           stat = "identity", fill = "tomato2") +
  labs(x = "Second Floor Apt.", y = "") +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(axis.text.x = element_text(size  =13, angle = 45, hjust = 1),
...
```{r, third_floor_apt_t, warning=FALSE, message=FALSE}

p_third_floor_apt_top <- third_floor_apt_top %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, conc.), y = conc.),
           stat = "identity", fill = "midnightblue") +
  labs(x = "Third Floor Apt.", y = "") +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 13, angle = 45, hjust = 1),
...
grid.arrange(p_basement_top, p_gallery_top, p_kitcehn_top, p_outdoor_top,
             p_second_floor_apt_top,p_third_floor_apt_top,
             ncol = 3, nrow = 2,
...
#gallery od ratio
gallery_top_or <- indoor %>% 
  filter(ID == "Gallery") %>% 
  group_by(analyte) %>% 
  arrange(desc(od_ratio))
gallery_top_or <- top_n(ungroup(gallery_top_or), 5, od_ratio)
...
kitchen_top_or <- indoor %>% 
  ...
  group_by(analyte) %>% 
  arrange(desc(od_ratio))
kitchen_top_or <- top_n(ungroup(kitchen_top_or), 5, od_ratio)

#second_floor_apt od ratio
second_floor_apt_top_or <- indoor %>% 
  filter(ID == "Second Floor Apt") %>%  
  group_by(analyte) %>% 
  arrange(desc(od_ratio))
second_floor_apt_top_or <- top_n(ungroup(second_floor_apt_top_or), 5, od_ratio)

#third_floor_apt od ratio
third_floor_apt_top_or <- indoor %>% 
  filter(ID == "Third Floor Apt") %>%  
  group_by(analyte) %>% 
  arrange(desc(od_ratio))
third_floor_apt_top_or <- top_n(ungroup(third_floor_apt_top_or), 5, od_ratio)
...
```{r gallery_or_t, warning=FALSE, message=FALSE}

p_gallery_top_or <- gallery_top_or %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, od_ratio), y = od_ratio),
           stat = "identity", fill = "chocolate4") +
  labs(x = "Gallery", y = "") +
...
```{r second_floor_apt_or_t, warning=FALSE, message=FALSE}

p_second_floor_apt_top_or <- second_floor_apt_top_or %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, od_ratio), y = od_ratio),
           stat = "identity", fill = "#50C878") +
  labs(x = "Second Floor Apt", y = "") +
...
# p_second_floor_apt_top_or + theme(
#   panel.background = element_rect(fill = "lightblue",
#                                 colour = "lightblue",
#                                 size = 0.5, linetype = "solid"))
...
```{r third_floor_apt_or_t, warning=FALSE, message=FALSE}

p_third_floor_apt_top_or <- third_floor_apt_top_or %>% 
  ggplot() +
  geom_bar(aes(x = reorder(analyte, od_ratio), y = od_ratio),
           stat = "identity", fill = "tomato2") +
  labs(x = "Third Floor Apt", y = "") +
...
# p_third_floor_apt_top_or + theme(
#   panel.background = element_rect(fill = "lightblue",
#                                 colour = "lightblue",
#                                 size = 0.5, linetype = "solid"))
...
grid.arrange(p_basement_top_or, p_gallery_top_or, p_kitchen_top_or,
             p_second_floor_apt_top_or, p_third_floor_apt_top_or,
             ncol = 3, nrow = 2,
