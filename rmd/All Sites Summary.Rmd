---
title: "Summa Canister Sites Summary, VOC Data"
author: "Dr. Carter, Will Clagett, Jade Lippmann"
date: "2022 Oct - 2024 May"
output: html_document
---
```{r packages, include=FALSE}
library(tidyverse)
library(plyr)
library(faraway)
library(cowplot)
library(reshape2)
library(corrplot)
library(scales)
library(ggthemes)
library(gridExtra)
library(patchwork)
library(gghighlight)
library(ggdark)
library(viridis)
library(DT)
library(plotly)
library(knitr)
library(magick)
library(readxl)

```

```{r, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
```

# **VOC Data**
## **All Sites (n=18)**

```{r, site_photo, out.width="50%"}

#include_graphics("../graphics/XXX.png")

```


---

## Data set
From October 2022 to May 2024 we deployed Summa Canisters at 18 of the 50 CDPHE
Project sites. To date 11 of the 18 sites have been analyzed by ATMOS and are 
summarized below. Almost all sites had a canister co-located with a QTrak and 
the outdoor monitors. Some sites had fewer canisters because of equipment
from the ATMOS lab. Each deployment was analyzed for 62 VOC and we have broken 
down the analytes into informal categories. We have also broken down the various
sample locations into room types. Below are plots summarizing all of sites by
Room type, season, and city/town. 


| CDPHE sites sampled organized by date |
|:------|
| 063 A & B. High Desert Assisted Living, Pagosa Springs |
| 079. Mango House, Denver |
| 066. Homeward Bound Shelter, Pagosa Springs |
| 085. La Puente, Alamosa |
| 086. BGCSLV, Alamosa |
| 099. Radiant Healthcare, Rocky Ford |
| 040. Teaching Tree (post), Fort Collins |
| 103. Karis Community, Denver |
| 107. The Center, Denver |
| 094. Ability Advocates, Pagosa Springs |
| 108. The Phoenix, Denver |
| 106. Lawrence St. Shelter, Denver |
| 109. Amaryllis, Denver |
| 105. Lawrence St. Center, Denver |
| 101. Silverkey, Colorado Springs |
| 104. CCS, Colorado Springs |
| 089. Cortez Library, Cortez |
| 002. Routt DC, Steamboat Springs |

| Room type categories |
|:------|
| 1. Outdoor |
| 2. Kitchens/Break rooms |
| 3. Offices/staff locations |
| 4. Classrooms |
| 5. Temp Living (shelters) |
| 6. Apartments/semi-permanent |
| 7. Lobbys/waiting rooms |
| 8. Recreation*/Gyms |
| 9. Medical/clinic exam rooms |
| 10. Other 


*includes open spaces for social gathering and general activity areas 

---

```{r load_data, include=FALSE}
#load data
sites <- read_excel("../data/voc_samples_all.xlsx")

#sites
#site 040
site_040 <- sites %>% 
  filter(site_id == "040")
#SITE 063 A
site_063A <- sites %>% 
  filter(site_id == "063 A")
#SITE 063 B
site_063B <- sites %>% 
  filter(site_id == "063 B")
#SITE 066
site_066 <- sites %>% 
  filter(site_id == "066")
# SITE 079
site_079 <- sites %>% 
  filter(site_id == "079")
# SITE 085
site_085 <- sites %>% 
  filter(site_id == "085")
# SITE 086
site_086 <- sites %>% 
  filter(site_id == "086")
# SITE 099
site_099 <- sites %>% 
  filter(site_id == "099")
# SITE 103
site_103 <- sites %>% 
  filter(site_id == "103")
# SITE 107
site_107 <- sites %>% 
  filter(site_id == "107")
# site 108
site_108 <- sites %>% 
  filter(site_id == "108")
# site 094
site_094 <- sites %>% 
  filter(site_id == "094")
#site 106
site_106 <- sites %>% 
  filter(site_id == "106")
# site 105
site_105 <- sites %>% 
  filter(site_id == "105")
# site 104
site_104 <- sites %>% 
  filter(site_id == "104")
#site 089
site_089 <- sites %>% 
  filter(site_id == "089")
# site 002
site_002 <- sites %>% 
  filter(site_id == "002")
# site 101
site_101 <- sites %>% 
  filter(site_id == "101")
# site 109
site_109 <- sites %>% 
  filter(site_id == "109")

#locations
table(sites$type)
#outdoor
outdoor <- sites %>% 
  filter(type == "Outdoor")
#kitchens
kitchens <- sites %>% 
  filter(type =="kitchen/dining")
#staff locations
offices <- sites %>% 
  filter(type %in% c("office" , "lounge"))
#classrooms
classrooms <- sites %>% 
  filter(type == "classroom")
#temp living
temp_living <- sites %>% 
  filter(type == "barrack")
#apartment
apartment <- sites %>% 
  filter(type == "apartment")
#lobbys
lobby <- sites %>% 
  filter(type == "lobby")
#others
other <- sites %>% 
  filter(type == "other")
#recreation
recreation <- sites %>% 
  filter(type == "recreation")
#medical
medical <- sites %>% 
  filter(type == "exam")

#data table
sites_table <- sites %>% 
  select(1:3,6,7,16)

```


## Data Table
```{r data_table, warning=FALSE,message=FALSE}
datatable(sites_table, colnames = c("Site ID", "Name", "Location", "Analyte",
                              "Concentration", "Category"),
          options = list(pageLenght = 10), rownames = FALSE,
          caption = "All Sites Table, Concentrations: ppb(v) or methane ppb(v)")

```

---

## Summary Plots of all VOC Locations

Outdoor Locations Plot
```{r outdoor_plotly, fig.align = "center", fig.dim=c(12,8), warning=FALSE, message=FALSE}
#outdoor concentrations
pc_outdoor <- ggplot(outdoor, aes(x = reorder(analyte, conc.),
                                  y = conc., color = site_id,
                     text = paste("Analyte: ", analyte,
                                  "<br> Room", room,
                                  "<br> Conc. :", conc.,
                                  "<br> Class: ", category))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  ggtitle("Outdoor Locations") +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1)) +
  labs(x = "Ananlyte", y = "Concentration") +
  scale_color_manual(name = "Site ID",
                     values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc"))
ggplotly(pc_outdoor, tooltip = "text")

```


Kitchens & Dining Rooms
```{r kitchens_plotly, fig.align = "center", fig.dim=c(12,8), warning=FALSE, message=FALSE}
#kitchen/dinning locations
pc_kitchens <- ggplot(kitchens, aes(x = reorder(analyte, conc.),
                                  y = conc., color = site_id,
                                  text = paste("Analyte: ", analyte,
                                               "<br> Room", room,
                                               "<br> Conc. :", conc.,
                                               "<br> Class: ", category))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  ggtitle("Kitchen & Dining Locations") +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1)) +
  labs(x = "Ananlyte", y = "Concentration") +
  scale_color_manual(name = "Site ID",
                     values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc"))
ggplotly(pc_kitchens, tooltip = "text")

```

Staff Locations
```{r staff_plotly, fig.align = "center", fig.dim=c(12,8), warning=FALSE, message=FALSE}

#staff locations
pc_offices <- ggplot(offices, aes(x = reorder(analyte, conc.),
                                  y = conc., color = site_id,
                                  text = paste("Analyte: ", analyte,
                                               "<br> Room", room,
                                               "<br> Conc. :", conc.,
                                               "<br> Class: ", category))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  ggtitle("Staff Locations") +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1)) +
  labs(x = "Ananlyte", y = "Concentration") +
  scale_color_manual(name = "Site ID",
                     values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc"))
ggplotly(pc_offices, tooltip = "text")

```

Classrooms
```{r classrooms_plotly, fig.align = "center", fig.dim=c(12,8), warning=FALSE, message=FALSE}

#classroom locations
pc_classrooms <- ggplot(classrooms, aes(x = reorder(analyte, conc.),
                                  y = conc., color = site_id,
                                  text = paste("Analyte: ", analyte,
                                               "<br> Room", room,
                                               "<br> Conc. :", conc.,
                                               "<br> Class: ", category))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  ggtitle("Classrooms") +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1)) +
  labs(x = "Ananlyte", y = "Concentration") +
  scale_color_manual(name = "Site ID",
                     values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc"))
ggplotly(pc_classrooms, tooltip = "text")


```

Temporary Living & Barracks Locations
```{r templiving_plotly, fig.align = "center", fig.dim=c(12,8), warning=FALSE, message=FALSE}

#temp living locations
pc_temp_living <- ggplot(temp_living, aes(x = reorder(analyte, conc.),
                                  y = conc., color = site_id,
                                  text = paste("Analyte: ", analyte,
                                               "<br> Room", room,
                                               "<br> Conc. :", conc.,
                                               "<br> Class: ", category))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  ggtitle("Temp Living Locations") +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1)) +
  labs(x = "Ananlyte", y = "Concentration") +
  scale_color_manual(name = "Site ID",
                     values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc"))
ggplotly(pc_temp_living, tooltip = "text")

```

Apartments & Semi-Permanent Locations
```{r apartments_plotly, fig.align = "center", fig.dim=c(12,8), warning=FALSE, message=FALSE}
#apartment locations
pc_apartment <- ggplot(apartment, aes(x = reorder(analyte, conc.),
                                  y = conc., color = site_id,
                                  text = paste("Analyte: ", analyte,
                                               "<br> Room", room,
                                               "<br> Conc. :", conc.,
                                               "<br> Class: ", category))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  ggtitle("Semi-Perminant Living Locations") +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1)) +
  labs(x = "Ananlyte", y = "Concentration") +
  scale_color_manual(name = "Site ID",
                     values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc"))
ggplotly(pc_apartment, tooltip = "text")


```

Lobbys & Building Waiting Areas
```{r lobbys_plotly, fig.align = "center", fig.dim=c(12,8), warning=FALSE, message=FALSE}

#lobby locations
pc_lobby <- ggplot(lobby, aes(x = reorder(analyte, conc.),
                                  y = conc., color = site_id,
                                  text = paste("Analyte: ", analyte,
                                               "<br> Room", room,
                                               "<br> Conc. :", conc.,
                                               "<br> Class: ", category))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  ggtitle("Lobbys & Waiting Rooms") +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1)) +
  labs(x = "Ananlyte", y = "Concentration") +
  scale_color_manual(name = "Site ID",
                     values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc"))
ggplotly(pc_lobby, tooltip = "text")


```

Recreation & Gyms
```{r recreation_plotly, fig.align = "center", fig.dim=c(12,8), warning=FALSE, message=FALSE}

#rec locations
pc_recreation <- ggplot(recreation, aes(x = reorder(analyte, conc.),
                                  y = conc., color = site_id,
                                  text = paste("Analyte: ", analyte,
                                               "<br> Room", room,
                                               "<br> Conc. :", conc.,
                                               "<br> Class: ", category))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  ggtitle("Recreation & Gym Locations") +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1)) +
  labs(x = "Ananlyte", y = "Concentration") +
  scale_color_manual(name = "Site ID",
                     values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc"))
ggplotly(pc_recreation, tooltip = "text")

```

Exam Rooms & Medical Rooms
```{r exam_plotly, fig.align = "center", fig.dim=c(12,8), warning=FALSE, message=FALSE}

#medical locations
pc_medical <- ggplot(medical, aes(x = reorder(analyte, conc.),
                                  y = conc., color = site_id,
                                  text = paste("Analyte: ", analyte,
                                               "<br> Room", room,
                                               "<br> Conc. :", conc.,
                                               "<br> Class: ", category))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  ggtitle("Exam Rooms & Medical Locations") +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1)) +
  labs(x = "Ananlyte", y = "Concentration") +
  scale_color_manual(name = "Site ID",
                     values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc"))
ggplotly(pc_medical, tooltip = "text")

```


Others & Miscellaneous Locations
```{r other_plotly, fig.align = "center", fig.dim=c(12,8), warning=FALSE, message=FALSE}

#other locations
pc_other <- ggplot(other, aes(x = reorder(analyte, conc.),
                                  y = conc., color = site_id,
                                  text = paste("Analyte: ", analyte,
                                               "<br> Room", room,
                                               "<br> Conc. :", conc.,
                                               "<br> Class: ", category))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  ggtitle("Miscellaneous Locations") +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1)) +
  labs(x = "Ananlyte", y = "Concentration") +
  scale_color_manual(name = "Site ID",
                     values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc"))
ggplotly(pc_other, tooltip = "text")

```


## Plots for each analyte category
```{r analyte_data, include=FALSE}

#create category objects
#Alcohol
alcohol <- sites %>%
  filter(category == "alcohol")
#Aldehyde
aldehyde <- sites %>% 
  filter(category == "aldehyde")
#Straight Chain
straight_chain <- sites %>% 
  filter(category == "straight chain")
#Aromatic
aromatic <- sites %>% 
  filter(category == "aromatic")
#Btex
btex <- sites %>% 
  filter(category == "btex")
#chlorinated
chlorinated <- sites %>% 
  filter(category == "chlorinated")
#Ketone
ketone <- sites %>% 
  filter(category == "ketone")
#Other
other <- sites %>% 
  filter(category == "other")

```

```{r alcohol_plot, warning=FALSE, message=FALSE}

p_alcohol <- ggplot(alcohol, aes(x = reorder(analyte, conc.),
                                 y = conc., color = site_id,
                                 text = paste("Analyte: ", analyte,
                                               "<br> Conc. :", conc.))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 10)) +
  labs(x = "", y = "") +
  ggtitle("Alcohols") +
  scale_color_manual(name = "Room ID",
                    values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc")) +
  theme(panel.grid.minor = element_line(linetype = "dashed"))

ggplotly(p_alcohol, tooltip = "text")

```


```{r aldehyde_plot, warning=FALSE, message=FALSE}

p_aldehyde <- ggplot(aldehyde, aes(x = reorder(analyte, conc.),
                                 y = conc., color = site_id,
                                 text = paste("Analyte: ", analyte,
                                               "<br> Conc. :", conc.))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 10)) +
  labs(x = "", y = "") +
  ggtitle("Aldehydes") +
  scale_color_manual(name = "Room ID",
                    values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc")) +
  theme(panel.grid.minor = element_line(linetype = "dashed"))

ggplotly(p_aldehyde, tooltip = "text")


```


```{r straight_chain_plot, fig.align = "center", out.width= "100%", warning=FALSE, message=FALSE}

p_straight_chain <- ggplot(straight_chain, aes(x = reorder(analyte, conc.),
                                 y = conc., color = site_id,
                                 text = paste("Analyte: ", analyte,
                                               "<br> Conc. :", conc.))) +
  geom_point(shape = 18, size = 2, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1)) +
  labs(x = "", y = "") +
  ggtitle("Straight Chains") +
  scale_color_manual(name = "Room ID",
                    values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc")) +
  theme(panel.grid.minor = element_line(linetype = "dashed"))

ggplotly(p_straight_chain, tooltip = "text")


```


```{r aromatic_plot, warning=FALSE, message=FALSE}

p_aromatic <- ggplot(aromatic, aes(x = reorder(analyte, conc.),
                                 y = conc., color = site_id,
                                 text = paste("Analyte: ", analyte,
                                               "<br> Conc. :", conc.))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 10)) +
  labs(x = "", y = "") +
  ggtitle("Aromatics") +
  scale_color_manual(name = "Room ID",
                    values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc")) +
  theme(panel.grid.minor = element_line(linetype = "dashed"))

ggplotly(p_aromatic, tooltip = "text")


```


```{r btex_plot, fig.align = "center", out.width= "100%", warning=FALSE, message=FALSE}

p_btex <- ggplot(btex, aes(x = reorder(analyte, conc.),
                                 y = conc., color = site_id,
                                 text = paste("Analyte: ", analyte,
                                               "<br> Conc. :", conc.))) +
  geom_point(shape = 18, size = 2, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 13, angle = 45, hjust = 1)) +
  labs(x = "", y = "") +
  ggtitle("Btex") +
  scale_color_manual(name = "Room ID",
                    values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc")) +
  theme(panel.grid.minor = element_line(linetype = "dashed"))

ggplotly(p_btex, tooltip = "text")

```


```{r chlorinated_plot, warning=FALSE, message=FALSE}

p_chlorinated <- ggplot(chlorinated, aes(x = reorder(analyte, conc.),
                                 y = conc., color = site_id,
                                 text = paste("Analyte: ", analyte,
                                               "<br> Conc. :", conc.))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 10)) +
  labs(x = "", y = "") +
  ggtitle("Chlorinated") +
  scale_color_manual(name = "Room ID",
                    values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc")) +
  theme(panel.grid.minor = element_line(linetype = "dashed"))

ggplotly(p_chlorinated, tooltip = "text")

```


```{r ketone_plot, message=FALSE, warning=FALSE}

p_ketone <- ggplot(ketone, aes(x = reorder(analyte, conc.),
                                 y = conc., color = site_id,
                                 text = paste("Analyte: ", analyte,
                                               "<br> Conc. :", conc.))) +
  geom_point(shape = 18, size = 3, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 10)) +
  labs(x = "", y = "") +
  ggtitle("Ketones") +
  scale_color_manual(name = "Room ID",
                    values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc")) +
  theme(panel.grid.minor = element_line(linetype = "dashed"))

ggplotly(p_ketone, tooltip = "text")

```


```{r other_plot, fig.align = "center", out.width= "100%", warning=FALSE, message=FALSE}

p_other <- ggplot(other, aes(x = reorder(analyte, conc.),
                                 y = conc., color = site_id,
                                 text = paste("Analyte: ", analyte,
                                               "<br> Conc. :", conc.))) +
  geom_point(shape = 18, size = 2, alpha = 0.5) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 13, angle = 45, hjust = 1)) +
  labs(x = "", y = "") +
  ggtitle("Other") +
  scale_color_manual(name = "Room ID",
                    values = c("#48bf8e", "#245a62", "#75b3d8", "#621da6",
                                "#e28de2", "#934270", "#e72fc2", "#5361c7",
                                "#b9cda1", "#096013", "#afe642", "#3aa609",
                                "#2af464", "#683d0d", "#efaa79", "#d6061a",
                                "#d9c937", "#9f04fc")) +
  theme(panel.grid.minor = element_line(linetype = "dashed"))

ggplotly(p_other, tooltip = "text")

```

## Plots for the Top 10 Analytes by Location
```{r top_ten_data, include=FALSE}

#040
s040_top <- site_040 %>% 
  group_by(conc.) %>% 
  arrange(desc(conc.))
s040_top <- top_n(ungroup(s040_top), 10, conc.)

```


```{r 040_t, warning=FALSE, message=FALSE}

p_s040_top <- s040_top %>% 
  ggplot(aes(x = reorder(analyte, conc.), y = conc.,
           color = room,
           text = paste("<br> Conc. :", conc.,
                        "<br> Class: ", category))) +
  geom_point(shape = 18, size = 5, alpha = 0.5) +
  labs(x = "", y = "") +
  ggtitle("Teaching Tree") +
  scale_y_log10(breaks = c(10e3, 10e2, 10e1, 10e0, 10e-1, 10e-2, 10e-3),
                labels = trans_format(`log10`, math_format(10^.x))) +
  theme_bw() +
  theme(axis.text = element_text(size = 13),
        axis.text.x = element_text(size = 13, angle = 45, hjust = 1),
        panel.grid.minor = element_line(linetype = "dashed")) +
  scale_color_manual(name = "Room ID",
                     values = c("orchid", "chocolate4", "goldenrod2","#50C878",
                                "tomato2", "midnightblue"))

ggplotly(p_s040_top, tooltip = "text")

```

---
